 ...
 */    class DOMStorageWrapper {
        ...
        set(key, value) {
            if (null == value) this.domStorage_.removeItem(this.prefixedName_(key)); else this.domStorage_.setItem(this.prefixedName_(key), index_esm2017_stringify(value));
        }
        get(key) {
 ...
 */    const logClient = new Logger("@firebase/database");
    ...
    const buildLogMessage_ = function(...varArgs) {
        ...
        for (let i = 0; i < varArgs.length; i++) {
            const arg = varArgs[i];
            if (Array.isArray(arg) || arg && "object" === typeof arg && "number" === typeof arg.length) message += buildLogMessage_.apply(null, arg); else if ("object" === typeof arg) message += index_esm2017_stringify(arg); else message += arg;
            message += " ";
        }
        return message;
    };
    ...
    const requireKey = function(key, obj) {
        if (obj && key in obj) return obj[key]; else throw new Error("Missing required key (" + key + ") in object: " + index_esm2017_stringify(obj));
    };
    const ObjectToUniqueKey = function(obj) {
        if ("object" !== typeof obj || null === obj) return index_esm2017_stringify(obj);
        const keys = [];
 ...
 */
    ...
    function queryParamsToRestQueryStringParameters(queryParams) {
        ...
        qs["orderBy"] = index_esm2017_stringify(orderBy);
        if (queryParams.startSet_) {
            qs["startAt"] = index_esm2017_stringify(queryParams.indexStartValue_);
            if (queryParams.startNameSet_) qs["startAt"] += "," + index_esm2017_stringify(queryParams.indexStartName_);
        }
        if (queryParams.endSet_) {
            qs["endAt"] = index_esm2017_stringify(queryParams.indexEndValue_);
            if (queryParams.endNameSet_) qs["endAt"] += "," + index_esm2017_stringify(queryParams.indexEndName_);
        }
 ...
 */    function getAuth(app = getApp()) {
    ...
    const auth = getAuth();
    const script_signUp = document.getElementById("signUp");
    ...
    function accountIsRegistred() {
        script_signUp.style.display = "none";
    ...
    function registration() {
        ...
        logout.style.display = "none";
        login.style.display = "none";
        script_signUp.style.display = "block";
    ...
    goSignIn.addEventListener("click", (e => {
        accountIsRegistred(script_signUp);
        goSignIn.style.display = "none";
    }));
    goSignUp.addEventListener("click", (e => {
        registration();
    }));
    script_signUp.addEventListener("click", (e => {
