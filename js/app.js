...
// //* Рендерим адреса пользователей, сохраняем в переменную
 ...
 */    const SDK_VERSION = version;
    function initializeApp(options, rawConfig = {}) {
        ...
        if (existingApp) if (deepEqual(options, existingApp.options) && deepEqual(config, existingApp.config)) return existingApp; else throw ERROR_FACTORY.create("duplicate-app", {
            appName: name
        });
 ...
 */    const MAX_HEADER_BYTES = 1024;
    ...
    class HeartbeatStorageImpl {
        ...
        async runIndexedDBEnvironmentCheck() {
            if (!isIndexedDBAvailable()) return false; else return validateIndexedDBOpenable().then((() => true)).catch((() => false));
        }
        async read() {
 ...
 */    class OnlineMonitor extends EventEmitter {
        constructor() {
            super([ "online" ]);
            this.online_ = true;
            if ("undefined" !== typeof window && "undefined" !== typeof window.addEventListener && !isMobileCordova()) {
 ...
 */    const RECONNECT_MIN_DELAY = 1e3;
    ...
    class PersistentConnection extends ServerActions {
        ...
        sendConnectStats_() {
            ...
            if (isMobileCordova()) stats["framework.cordova"] = 1; else if (isReactNative()) stats["framework.reactnative"] = 1;
 ...
 */
    ...
    function _logError(msg, ...args) {
        if (index_90ebcfae_logClient.logLevel <= LogLevel.ERROR) index_90ebcfae_logClient.error(`Auth (${SDK_VERSION}): ${msg}`, ...args);
    }
    /**
 * @license
 * Copyright 2020 Google LLC
 ...
 */    function initializeAuth(app, deps) {
        ...
        if (provider.isInitialized()) {
            ...
            if (deepEqual(initialOptions, null !== deps && void 0 !== deps ? deps : {})) return auth; else _fail(auth, "already-initialized");
        }
 ...
 * limitations under the License.
 */    function _isOnline() {
        if ("undefined" !== typeof navigator && navigator && "onLine" in navigator && "boolean" === typeof navigator.onLine && (_isHttpOrHttps() || isBrowserExtension() || "connection" in navigator)) return navigator.onLine;
        return true;
    }
 ...
 */    function _getBrowserName(userAgent) {
    ...
    function _isIE10() {
        return isIE() && 10 === document.documentMode;
    }
