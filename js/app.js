(() => {
    ...
    function modalVisible(message) {
        ...
        modalContent.style.animation = "loadAlert .3s linear 0s 1 forwards";
        modalMesssage.innerHTML = message;
        setTimeout((() => {
            modalContent.style.animation = "closeAlert .3s linear 0s 1 forwards";
        }), 2e3);
        setTimeout((() => {
 ...
 */
    ...
    function index_esm2017_isMobileCordova() {
        return "undefined" !== typeof window && !!(window["cordova"] || window["phonegap"] || window["PhoneGap"]) && /ios|iphone|ipod|ipad|android|blackberry|iemobile/i.test(index_esm2017_getUA());
    }
    ...
    function index_esm2017_deepEqual(a, b) {
        ...
        for (const k of aKeys) {
            ...
            if (isObject(aProp) && isObject(bProp)) {
                if (!index_esm2017_deepEqual(aProp, bProp)) return false;
 ...
 */
    ...
    class Logger {
        ...
        set logLevel(val) {
            if (!(val in index_esm2017_LogLevel)) throw new TypeError(`Invalid value "${val}" assigned to \`logLevel\``);
            this._logLevel = val;
        }
        setLogLevel(val) {
        ...
        log(...args) {
            this._userLogHandler && this._userLogHandler(this, index_esm2017_LogLevel.VERBOSE, ...args);
 ...
 */    const SDK_VERSION = version;
    function initializeApp(options, rawConfig = {}) {
        ...
        if (existingApp) if (index_esm2017_deepEqual(options, existingApp.options) && index_esm2017_deepEqual(config, existingApp.config)) return existingApp; else throw ERROR_FACTORY.create("duplicate-app", {
            appName: name
        });
 ...
 */
    ...
    async function readHeartbeatsFromIndexedDB(app) {
        ...
        try {
            ...
            if (e instanceof FirebaseError) logger.warn(e.message); else {
 ...
 */    const MAX_HEADER_BYTES = 1024;
    ...
    class HeartbeatStorageImpl {
        ...
        async runIndexedDBEnvironmentCheck() {
            if (!index_esm2017_isIndexedDBAvailable()) return false; else return validateIndexedDBOpenable().then((() => true)).catch((() => false));
        }
        async read() {
 ...
 */    registerVersion(index_esm_name, index_esm_version, "app");
    ...
    const index_esm2017_version = "0.13.5";
    /**
 * @license
 * Copyright 2019 Google LLC
 *
 ...
 */    class DOMStorageWrapper {
        ...
        set(key, value) {
            if (null == value) this.domStorage_.removeItem(this.prefixedName_(key)); else this.domStorage_.setItem(this.prefixedName_(key), stringify(value));
        }
        get(key) {
            const storedVal = this.domStorage_.getItem(this.prefixedName_(key));
            if (null == storedVal) return null; else return jsonEval(storedVal);
        }
        remove(key) {
 ...
 */    class MemoryStorage {
        ...
        get(key) {
            if (index_esm2017_contains(this.cache_, key)) return this.cache_[key];
            return null;
        }
        remove(key) {
 ...
 */    const logClient = new Logger("@firebase/database");
    ...
    const buildLogMessage_ = function(...varArgs) {
        ...
        for (let i = 0; i < varArgs.length; i++) {
            const arg = varArgs[i];
            if (Array.isArray(arg) || arg && "object" === typeof arg && "number" === typeof arg.length) message += buildLogMessage_.apply(null, arg); else if ("object" === typeof arg) message += stringify(arg); else message += arg;
            message += " ";
        }
        return message;
    };
    ...
    const enableLogging$1 = function(logger_, persistent) {
        index_esm2017_assert(!persistent || true === logger_ || false === logger_, "Can't turn on custom loggers persistently.");
        if (true === logger_) {
            logClient.logLevel = index_esm2017_LogLevel.VERBOSE;
            index_esm2017_logger = logClient.log.bind(logClient);
            if (persistent) SessionStorage.set("logging_enabled", true);
        } else if ("function" === typeof logger_) index_esm2017_logger = logger_; else {
    ...
    const log = function(...varArgs) {
        if (true === firstLog_) {
            firstLog_ = false;
            if (null === index_esm2017_logger && true === SessionStorage.get("logging_enabled")) enableLogging$1(true);
        }
    ...
    const executeWhenDOMReady = function(fn) {
        if (isNodeSdk() || "complete" === document.readyState) fn(); else {
            let called = false;
    ...
    const requireKey = function(key, obj) {
        if (obj && key in obj) return obj[key]; else throw new Error("Missing required key (" + key + ") in object: " + stringify(obj));
    };
    const ObjectToUniqueKey = function(obj) {
        if ("object" !== typeof obj || null === obj) return stringify(obj);
        const keys = [];
    ...
    const doubleToIEEE754String = function(v) {
        index_esm2017_assert(!isInvalidJSONNumber(v), "Invalid JSON number");
 ...
 */    class RepoInfo {
    ...
    function repoInfoConnectionURL(repoInfo, type, params) {
        index_esm2017_assert("string" === typeof type, "typeof type must == string");
        index_esm2017_assert("object" === typeof params, "typeof params must == object");
        let connURL;
 ...
 */    class StatsCollection {
        ...
        incrementCounter(name, amount = 1) {
            if (!index_esm2017_contains(this.counters_, name)) this.counters_[name] = 0;
 ...
 */    const FIREBASE_LONGPOLL_START_PARAM = "start";
    ...
    class BrowserPollConnection {
        ...
        static isAvailable() {
            if (isNodeSdk()) return false; else if (BrowserPollConnection.forceAllow_) return true; else return !BrowserPollConnection.forceDisallow_ && "undefined" !== typeof document && null != document.createElement && !isChromeExtensionContentScript() && !isWindowsStoreApp();
        }
    ...
    class FirebaseIFrameScriptHolder {
        ...
        addTag(url, loadCB) {
            if (isNodeSdk()) this.doNodeLongPoll(url, loadCB); else setTimeout((() => {
                try {
 ...
 */    const WEBSOCKET_MAX_FRAME_SIZE = 16384;
    ...
    class WebSocketConnection {
        ...
        static connectionURL_(repoInfo, transportSessionId, lastSessionId, appCheckToken, applicationId) {
            const urlParams = {};
            urlParams[VERSION_PARAM] = PROTOCOL_VERSION;
            if (!isNodeSdk() && "undefined" !== typeof location && location.hostname && FORGE_DOMAIN_RE.test(location.hostname)) urlParams[REFERER_PARAM] = FORGE_REF;
        ...
        extractFrameCount_(data) {
            index_esm2017_assert(null === this.frames, "We already have a frame buffer");
            if (data.length <= 6) {
 ...
 */    class EventEmitter {
        ...
        validateEventType_(eventType) {
            index_esm2017_assert(this.allowedEvents_.find((et => et === eventType)), "Unknown event: " + eventType);
        }
    }
    /**
 * @license
 ...
 */    class OnlineMonitor extends EventEmitter {
        constructor() {
            super([ "online" ]);
            this.online_ = true;
            if ("undefined" !== typeof window && "undefined" !== typeof window.addEventListener && !index_esm2017_isMobileCordova()) {
        ...
        getInitialEvent(eventType) {
            index_esm2017_assert("online" === eventType, "Unknown event type: " + eventType);
            return [ this.online_ ];
        }
        currentlyOnline() {
            return this.online_;
 ...
 */    class VisibilityMonitor extends EventEmitter {
        ...
        getInitialEvent(eventType) {
            index_esm2017_assert("visible" === eventType, "Unknown event type: " + eventType);
 ...
 */    const RECONNECT_MIN_DELAY = 1e3;
    ...
    class PersistentConnection extends ServerActions {
        ...
        static warnOnListenWarnings_(payload, query) {
            if (payload && "object" === typeof payload && index_esm2017_contains(payload, "w")) {
        ...
        sendConnectStats_() {
            const stats = {};
            let clientName = "js";
            if (isNodeSdk()) if (this.repoInfo_.nodeAdmin) clientName = "admin_node"; else clientName = "node";
            stats["sdk." + clientName + "." + index_esm2017_SDK_VERSION.replace(/\./g, "-")] = 1;
            if (index_esm2017_isMobileCordova()) stats["framework.cordova"] = 1; else if (index_esm2017_isReactNative()) stats["framework.reactnative"] = 1;
 ...
 */    let __EMPTY_NODE;
    class KeyIndex extends Index {
        ...
        makePost(indexValue, name) {
            index_esm2017_assert("string" === typeof indexValue, "KeyIndex indexValue must always be a string.");
 ...
 */    let MAX_NODE$2;
    ...
    const validatePriorityNode = function(priorityNode) {
        if (priorityNode.isLeafNode()) {
            ...
            index_esm2017_assert("string" === typeof val || "number" === typeof val || "object" === typeof val && index_esm2017_contains(val, ".sv"), "Priority must be a string or number.");
        } else index_esm2017_assert(priorityNode === MAX_NODE$2 || priorityNode.isEmpty(), "priority of unexpected type.");
 ...
 */    let __childrenNodeConstructor;
    class LeafNode {
        constructor(value_, priorityNode_ = LeafNode.__childrenNodeConstructor.EMPTY_NODE) {
            ...
            this.lazyHash_ = null;
            index_esm2017_assert(void 0 !== this.value_ && null !== this.value_, "LeafNode shouldn't be created with null/undefined value.");
        ...
        updateChild(path, newChildNode) {
            ...
            if (null === front) return newChildNode; else if (newChildNode.isEmpty() && ".priority" !== front) return this; else {
                index_esm2017_assert(".priority" !== front || 1 === pathGetLength(path), ".priority must be the last token in a path");
 ...
 */    let EMPTY_NODE;
    class ChildrenNode {
        constructor(children_, priorityNode_, indexMap_) {
            ...
            if (this.children_.isEmpty()) index_esm2017_assert(!this.priorityNode_ || this.priorityNode_.isEmpty(), "An empty node cannot have a priority");
        }
        ...
        updateChild(path, newChildNode) {
            ...
            if (null === front) return newChildNode; else {
                index_esm2017_assert(".priority" !== pathGetFront(path) || 1 === pathGetLength(path), ".priority must be the last token in a path");
 ...
 */    const USE_HINZE = true;
    function nodeFromJSON(json, priority = null) {
        ...
        index_esm2017_assert(null === priority || "string" === typeof priority || "number" === typeof priority || "object" === typeof priority && ".sv" in priority, "Invalid priority type found: " + typeof priority);
        ...
        if (!(json instanceof Array) && USE_HINZE) {
            ...
            each(json, ((key, childData) => {
                if (index_esm2017_contains(json, key)) if ("." !== key.substring(0, 1)) {
 ...
 */    class PathIndex extends Index {
        constructor(indexPath_) {
            ...
            index_esm2017_assert(!pathIsEmpty(indexPath_) && ".priority" !== pathGetFront(indexPath_), "Can't create PathIndex with empty path or .priority key");
        }
        extractChild(snap) {
 ...
 */    class QueryParams {
    ...
    function queryParamsToRestQueryStringParameters(queryParams) {
        ...
        if (queryParams.startSet_) {
            ...
            if (queryParams.startNameSet_) qs["startAt"] += "," + stringify(queryParams.indexStartName_);
        }
        if (queryParams.endSet_) {
            qs["endAt"] = stringify(queryParams.indexEndValue_);
            if (queryParams.endNameSet_) qs["endAt"] += "," + stringify(queryParams.indexEndName_);
        }
 ...
 */    class ReadonlyRestClient extends ServerActions {
        ...
        restRequest_(pathString, queryStringParameters = {}, callback) {
            ...
            return Promise.all([ this.authTokenProvider_.getToken(false), this.appCheckTokenProvider_.getToken(false) ]).then((([authToken, appCheckToken]) => {
                ...
                const url = (this.repoInfo_.secure ? "https://" : "http://") + this.repoInfo_.host + pathString + "?" + "ns=" + this.repoInfo_.namespace + index_esm2017_querystring(queryStringParameters);
 ...
 */    class EventGenerator {
    ...
    function eventGeneratorCompareChanges(eventGenerator, a, b) {
        if (null == a.childName || null == b.childName) throw assertionError("Should only compare child_ events.");
        ...
        if (null != writeTree.value) return node.updateChild(relativePath, writeTree.value); else {
            ...
            writeTree.children.inorderTraversal(((childKey, childTree) => {
                if (".priority" === childKey) {
                    index_esm2017_assert(null !== childTree.value, "Priority writes must always be leaf nodes");
 ...
 */    function newViewProcessor(filter) {
    ...
    function viewProcessorApplyOperation(viewProcessor, oldViewCache, operation, writesCache, completeCache) {
        ...
        } else if (operation.type === OperationType.LISTEN_COMPLETE) newViewCache = viewProcessorListenComplete(viewProcessor, oldViewCache, operation.path, writesCache, accumulator); else throw assertionError("Unknown operation type: " + operation.type);
    ...
    function viewProcessorGenerateEventCacheAfterServerEvent(viewProcessor, viewCache, changePath, writesCache, source, accumulator) {
        ...
        if (null != writeTreeRefShadowingWrite(writesCache, changePath)) return viewCache; else {
            ...
            if (pathIsEmpty(changePath)) {
                ...
                if (".priority" === childKey) {
                    index_esm2017_assert(1 === pathGetLength(changePath), "Can't have a priority with additional path components");
 ...
 */    class View {
    ...
    function viewRemoveEventRegistration(view, eventRegistration, cancelError) {
        const cancelEvents = [];
        if (cancelError) {
            index_esm2017_assert(null == eventRegistration, "A cancel should cancel all event registrations.");
    ...
    function viewApplyOperation(view, operation, writesCache, completeServerCache) {
        ...
        index_esm2017_assert(result.viewCache.serverCache.isFullyInitialized() || !oldViewCache.serverCache.isFullyInitialized(), "Once a server snap is complete, it should never go back");
 ...
 */    let referenceConstructor$1;
    ...
    function syncPointApplyOperation(syncPoint, operation, writesCache, optCompleteServerCache) {
        ...
        if (null !== queryId) {
            ...
            index_esm2017_assert(null != view, "SyncTree gave us an op for an invalid query.");
 ...
 */    let referenceConstructor;
    ...
    function syncTreeParseQueryKey_(queryKey) {
        ...
        index_esm2017_assert(-1 !== splitIndex && splitIndex < queryKey.length - 1, "Bad queryKey.");
        return {
    ...
    function syncTreeSetupListener_(syncTree, query, view) {
        ...
        const subtree = syncTree.syncPointTree_.subtree(path);
        if (tag) index_esm2017_assert(!syncPointHasCompleteView(subtree.value), "If we're adding a query, it shouldn't be shadowed"); else {
 ...
 */    class ExistingValueProvider {
    ...
    const resolveDeferredLeafValue = function(value, existingVal, serverValues) {
        ...
        if ("string" === typeof value[".sv"]) return resolveScalarDeferredValue(value[".sv"], existingVal, serverValues); else if ("object" === typeof value[".sv"]) return resolveComplexDeferredValue(value[".sv"], existingVal); else index_esm2017_assert(false, "Unexpected server value: " + JSON.stringify(value, null, 2));
    };
    ...
    const resolveComplexDeferredValue = function(op, existing, unused) {
        if (!op.hasOwnProperty("increment")) index_esm2017_assert(false, "Unexpected server value: " + JSON.stringify(op, null, 2));
        const delta = op["increment"];
        if ("number" !== typeof delta) index_esm2017_assert(false, "Unexpected increment value: " + delta);
        const existingNode = existing.node();
        index_esm2017_assert(null !== existingNode && "undefined" !== typeof existingNode, "Expected ChildrenNode.EMPTY_NODE for nulls");
 ...
 */    const INVALID_KEY_REGEX_ = /[\[\].#$\/\u0000-\u001F\u007F]/;
    ...
    const isValidPriority = function(priority) {
        return null === priority || "string" === typeof priority || "number" === typeof priority && !isInvalidJSONNumber(priority) || priority && "object" === typeof priority && index_esm2017_contains(priority, ".sv");
    };
    ...
    const validateFirebaseData = function(errorPrefix, data, path_) {
        ...
        if ("string" === typeof data && data.length > MAX_LEAF_SIZE_ / 3 && stringLength(data) > MAX_LEAF_SIZE_) throw new Error(errorPrefix + "contains a string greater than " + MAX_LEAF_SIZE_ + " utf8 bytes " + validationPathToErrorString(path) + " ('" + data.substring(0, 50) + "...')");
        if (data && "object" === typeof data) {
            ...
            each(data, ((key, value) => {
                if (".value" === key) hasDotValue = true; else if (".priority" !== key && ".sv" !== key) {
                    hasActualChild = true;
                    if (!index_esm2017_isValidKey(key)) throw new Error(errorPrefix + " contains an invalid key (" + key + ") " + validationPathToErrorString(path) + ".  Keys must be non-empty strings " + 'and can\'t contain ".", "#", "$", "/", "[", or "]"');
                }
    ...
    const validateFirebaseMergePaths = function(errorPrefix, mergePaths) {
        ...
        for (i = 0; i < mergePaths.length; i++) {
            ...
            for (let j = 0; j < keys.length; j++) if (".priority" === keys[j] && j === keys.length - 1) ; else if (!index_esm2017_isValidKey(keys[j])) throw new Error(errorPrefix + "contains an invalid key (" + keys[j] + ") in path " + curPath.toString() + ". Keys must be non-empty strings " + 'and can\'t contain ".", "#", "$", "/", "[", or "]"');
        }
    ...
    const validatePathString = function(fnName, argumentName, pathString, optional) {
        ...
        if (!isValidPathString(pathString)) throw new Error(index_esm2017_errorPrefix(fnName, argumentName) + 'was an invalid path = "' + pathString + '". Paths must be non-empty strings and ' + 'can\'t contain ".", "#", "$", "[", or "]"');
    };
    ...
    const validateUrl = function(fnName, parsedUrl) {
        ...
        if (!("string" === typeof parsedUrl.repoInfo.host) || 0 === parsedUrl.repoInfo.host.length || !index_esm2017_isValidKey(parsedUrl.repoInfo.namespace) && "localhost" !== parsedUrl.repoInfo.host.split(":")[0] || 0 !== pathString.length && !isValidRootPathString(pathString)) throw new Error(index_esm2017_errorPrefix(fnName, "url") + "must be a valid firebase URL and " + 'the path can\'t contain ".", "#", "$", "[", or "]".');
    };
    /**
 * @license
 * Copyright 2017 Google LLC
    ...
    function eventListRaise(eventList) {
        for (let i = 0; i < eventList.events.length; i++) {
            ...
            if (null !== eventData) {
                ...
                if (index_esm2017_logger) log("event: " + eventData.toString());
 ...
 */    const INTERRUPT_REASON = "repo_interrupt";
    ...
    function repoSendTransactionQueue(repo, path, queue) {
        ...
        for (let i = 0; i < queue.length; i++) {
            const txn = queue[i];
            index_esm2017_assert(0 === txn.status, "tryToSendTransactionQueue_: items in queue should all be run.");
            txn.status = 1;
            txn.retryCount++;
    ...
    function repoRerunTransactionQueue(repo, queue, path) {
        ...
        for (let i = 0; i < queue.length; i++) {
            ...
            index_esm2017_assert(null !== relativePath, "rerunTransactionsUnderNode_: relativePath should not be null.");
            if (4 === transaction.status) {
                ...
                if (void 0 !== newData) {
                    ...
                    const hasExplicitPriority = "object" === typeof newData && null != newData && index_esm2017_contains(newData, ".priority");
    ...
    function repoAbortTransactionsOnNode(repo, node) {
        ...
        if (queue) {
            ...
            for (let i = 0; i < queue.length; i++) if (3 === queue[i].status) ; else if (1 === queue[i].status) {
                ...
                index_esm2017_assert(0 === queue[i].status, "Unexpected transaction status in abort");
                queue[i].unwatcher();
 ...
 */    const FIREBASE_DATABASE_EMULATOR_HOST_VAR = "FIREBASE_DATABASE_EMULATOR_HOST";
    ...
    function repoManagerDatabaseFromApp(app, authProvider, appCheckProvider, url, nodeAdmin) {
        ...
        let isEmulator;
        let dbEmulatorHost;
        if ("undefined" !== typeof process && process.env) dbEmulatorHost = process.env[FIREBASE_DATABASE_EMULATOR_HOST_VAR];
        if (dbEmulatorHost) {
            isEmulator = true;
    ...
    function getDatabase(app = index_esm2017_getApp(), url) {
        return index_esm2017_getProvider(app, "database").getImmediate({
            identifier: url
        });
    }
    /**
 ...
 * limitations under the License.
 */
    const index_90ebcfae_logClient = new Logger("@firebase/auth");
    function _logError(msg, ...args) {
        if (index_90ebcfae_logClient.logLevel <= index_esm2017_LogLevel.ERROR) index_90ebcfae_logClient.error(`Auth (${SDK_VERSION}): ${msg}`, ...args);
    }
    /**
 * @license
 * Copyright 2020 Google LLC
 ...
 */    function initializeAuth(app, deps) {
        ...
        if (provider.isInitialized()) {
            ...
            if (index_esm2017_deepEqual(initialOptions, null !== deps && void 0 !== deps ? deps : {})) return auth; else _fail(auth, "already-initialized");
        }
 ...
 * limitations under the License.
 */    function _isOnline() {
        if ("undefined" !== typeof navigator && navigator && "onLine" in navigator && "boolean" === typeof navigator.onLine && (_isHttpOrHttps() || index_esm2017_isBrowserExtension() || "connection" in navigator)) return navigator.onLine;
        return true;
    }
 ...
 */    async function _logoutIfInvalidated(user, promise, bypassAuthState = false) {
        ...
        try {
            ...
            if (e instanceof FirebaseError && isUserInvalidated(e)) if (user.auth.currentUser === user) await user.auth.signOut();
            throw e;
        }
    }
 ...
 */    function _getBrowserName(userAgent) {
    ...
    function _isIE10() {
        return index_esm2017_isIE() && 10 === document.documentMode;
    }
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */    const index_90ebcfae_DB_NAME = "firebaseLocalStorageDb";
    const index_90ebcfae_DB_VERSION = 1;
    ...
    function _deleteDatabase() {
        const request = indexedDB.deleteDatabase(index_90ebcfae_DB_NAME);
        return new DBPromise(request).toPromise();
    }
    function _openDatabase() {
        const request = indexedDB.open(index_90ebcfae_DB_NAME, index_90ebcfae_DB_VERSION);
        return new Promise(((resolve, reject) => {
 ...
 */    const WIDGET_PATH = "__/auth/handler";
    ...
    function _getRedirectUrl(auth, provider, authType, redirectUrl, eventId, additionalParams) {
        ...
        if (provider instanceof FederatedAuthProvider) {
            ...
            if (!index_esm2017_isEmpty(provider.getCustomParameters())) params.customParameters = JSON.stringify(provider.getCustomParameters());
 ...
 */    const WEB_STORAGE_SUPPORT_KEY = "webStorageSupport";
    ...
    var index_90ebcfae_version = "0.20.5";
    /**
 * @license
 * Copyright 2020 Google LLC
 *
 ...
 */    function getVersionForPlatform(clientPlatform) {
    ...
    function registerAuth(clientPlatform) {
        ...
        registerVersion(index_90ebcfae_name, index_90ebcfae_version, getVersionForPlatform(clientPlatform));
        registerVersion(index_90ebcfae_name, index_90ebcfae_version, "esm2017");
    }
    /**
 * @license
 * Copyright 2021 Google LLC
 ...
 */    function getAuth(app = index_esm2017_getApp()) {
    ...
    function A(a, b) {
        ...
        if (a) {
            ...
            d ? (this.clientX = void 0 !== d.clientX ? d.clientX : d.pageX, this.clientY = void 0 !== d.clientY ? d.clientY : d.pageY, 
            this.screenX = d.screenX || 0, this.screenY = d.screenY || 0) : (this.clientX = void 0 !== a.clientX ? a.clientX : a.pageX, 
    ...
    $a.prototype.add = function(a, b, c, d, e) {
        ...
        a || (a = this.g[f] = [], this.h++);
        var h = ab(a, b, d, e);
        -1 < h ? (b = a[h], c || (b.fa = !1)) : (b = new Ya(b, this.src, f, !!d, e), b.fa = c, 
        a.push(b));
        return b;
    };
    function bb(a, b) {
    ...
    function nb(a, b, c, d, e) {
        if (Array.isArray(b)) for (var f = 0; f < b.length; f++) nb(a, b[f], c, d, e); else (d = p(d) ? !!d.capture : !!d, 
        c = hb(c), a && a[B]) ? (a = a.i, b = String(b).toString(), b in a.g && (f = a.g[b], 
        c = ab(f, c, d, e), -1 < c && (Za(f[c]), Array.prototype.splice.call(f, c, 1), 0 == f.length && (delete a.g[b], 
        a.h--)))) : a && (a = jb(a)) && (b = a.g[b.toString()], a = -1, b && (a = ab(b, c, d, e)), 
        (c = -1 < a ? b[a] : null) && ob(c));
    }
    function ob(a) {
    ...
    k.kb = function() {
        if (this.da) {
            ...
            0 < a && a < .8 * this.h ? this.S = this.g.setTimeout(this.j, this.h - a) : (this.S && (this.g.clearTimeout(this.S), 
            this.S = null), D(this, "tick"), this.da && (Fb(this), this.start()));
        }
    };
    k.start = function() {
        this.da = !0;
    ...
    function kc(a, b) {
        ...
        b = a.H ? ya(a.H) : {};
        a.s ? (a.u || (a.u = "POST"), b["Content-Type"] = "application/x-www-form-urlencoded", 
        a.g.ea(a.A, a.u, a.s, b)) : (a.u = "GET", a.g.ea(a.A, a.u, null, b));
        I(1);
                ...
                if (!(3 > r) && (3 != r || Ja || this.g && (this.h.h || this.g.ga() || oc(this.g)))) {
                    ...
                    if (this.i) {
                        if (this.$ && !this.J) {
                            ...
                            if (c = m) F(this.j, this.m, c, "Initial handshake response via X-HTTP-Initial-Response"), 
                            this.J = !0, sc(this, c); else {
                                this.i = !1;
                                this.o = 3;
                                J(12);
                                P(this);
                    ...
                    } else 400 == c && 0 < h.indexOf("Unknown SID") ? (this.o = 3, J(12)) : (this.o = 0, 
                    J(13)), P(this), rc(this);
                }
            }
        } catch (r) {}
    ...
    function tc(a, b, c) {
        ...
        a.i = a.i && d;
        d ? 0 < c.length && !a.aa && (a.aa = !0, b = a.l, b.g == a && b.$ && !b.L && (b.h.info("Great, no buffering proxy detected. Bytes received: " + c.length), 
        wc(b), b.L = !0, J(11))) : (F(a.j, a.m, c, "[Invalid Chunked Response]"), P(a), 
        rc(a));
    }
    k.fb = function() {
        if (this.g) {
    ...
    k.eb = function() {
        this.B = null;
        const a = Date.now();
        0 <= a - this.Y ? (Qb(this.j, this.A), 2 != this.K && (I(3), J(17)), P(this), this.o = 2, 
        rc(this)) : xc(this, this.Y - a);
    };
    function rc(a) {
            ...
            if (0 != c.G && (c.g == a || yc(c.i, a))) if (c.I = a.N, !a.J && yc(c.i, a) && 3 == c.G) {
                ...
                if (2 == c.G) if ("c" == m[0]) {
                    ...
                    if (ca) {
                        ...
                        if (Ea) {
                            var f = d.i;
                            !f.g && (w(Ea, "spdy") || w(Ea, "quic") || w(Ea, "h2")) && (f.j = f.l, f.g = new Set, 
                            f.h && (Dc(f, f.h), f.h = null));
                        }
                        if (d.D) {
                    ...
                    0 < c.l.length && Hc(c);
                } else "stop" != m[0] && "close" != m[0] || Q(c, 7); else 3 == c.G && ("stop" == m[0] || "close" == m[0] ? "stop" == m[0] ? Q(c, 7) : Ic(c) : "noop" != m[0] && c.j && c.j.wa(m), 
                c.A = 0);
            }
            I(4);
        } catch (m) {}
    ...
    function Lc(a) {
        ...
        if (a.i != a.g.length) {
            var e = {};
            for (c = b = 0; b < a.g.length; ) d = a.g[b], T(e, d) || (a.g[c++] = d, e[d] = 1), 
            b++;
            a.g.length = c;
        }
    }
    ...
    function U(a, b) {
        ...
        if (a instanceof U) {
            ...
            Sc(this, c);
            this.o = a.o;
        } else a && (c = String(a).match(Mc)) ? (this.g = !!b, Oc(this, c[1] || "", !0), 
        this.s = Tc(c[2] || ""), Pc(this, c[3] || "", !0), Qc(this, c[4]), this.l = Tc(c[5] || "", !0), 
        Sc(this, c[6] || "", !0), this.o = Tc(c[7] || "")) : (this.g = !!b, this.h = new Rc(null, this.g));
    }
    U.prototype.toString = function() {
        var a = [], b = this.j;
        b && a.push(Uc(b, Vc, !0), ":");
        var c = this.i;
        if (c || "file" == b) a.push("//"), (b = this.s) && a.push(Uc(b, Vc, !0), "@"), 
        a.push(encodeURIComponent(String(c)).replace(/%25([0-9a-fA-F]{2})/g, "%$1")), c = this.m, 
    ...
    function Uc(a, b, c) {
        return "string" === typeof a ? (a = encodeURI(a).replace(b, cd), c && (a = a.replace(/%25([0-9a-fA-F]{2})/g, "%$1")), 
        a) : null;
    }
    function cd(a) {
        a = a.charCodeAt(0);
    ...
    function dd(a, b) {
        V(a);
        b = W(a, b);
        T(a.g.h, b) && (a.i = null, a.h -= a.g.get(b).length, a = a.g, T(a.h, b) && (delete a.h[b], 
        a.i--, a.g.length > 2 * a.i && Lc(a)));
    }
    function ed(a, b) {
        V(a);
        b = W(a, b);
    ...
    function gd(a) {
        this.l = a || hd;
        l.PerformanceNavigationTiming ? (a = l.performance.getEntriesByType("navigation"), 
        a = 0 < a.length && ("hq" == a[0].nextHopProtocol || "h2" == a[0].nextHopProtocol)) : a = !!(l.g && l.g.Ea && l.g.Ea() && l.g.Ea().Zb);
        this.j = a ? this.l : 1;
        this.g = null;
    ...
    k.Va = function(a) {
        if (this.g && (this.l = a, this.h || (this.status = this.l.status, this.statusText = this.l.statusText, 
        this.h = a.headers, this.readyState = 2, sd(this)), this.g && (this.readyState = 3, 
        sd(this), this.g))) if ("arraybuffer" === this.responseType) a.arrayBuffer().then(this.Ta.bind(this), this.ha.bind(this)); else if ("undefined" !== typeof l.ReadableStream && "body" in a) {
    ...
    k.ea = function(a, b, c, d) {
        ...
        try {
            Ad(this), 0 < this.B && ((this.K = Bd(this.g)) ? (this.g.timeout = this.B, this.g.ontimeout = q(this.pa, this)) : this.A = Gb(this.pa, this.B, this)), 
            this.v = !0, this.g.send(a), this.v = !1;
        } catch (f) {
            zd(this, f);
        }
    };
    ...
    k.pa = function() {
        "undefined" != typeof goog && this.g && (this.j = "Timed out after " + this.B + "ms, aborting", 
        this.m = 8, D(this, "timeout"), this.abort(8));
    };
    function zd(a, b) {
        a.h = !1;
    ...
    k.abort = function(a) {
        this.g && this.h && (this.h = !1, this.l = !0, this.g.abort(), this.l = !1, this.m = a || 7, 
        D(this, "complete"), D(this, "abort"), Dd(this));
    };
    k.M = function() {
    ...
    function Ed(a) {
        if (a.h && "undefined" != typeof goog && (!a.C[1] || 4 != O(a) || 2 != a.ba())) if (a.v && 4 == O(a)) Gb(a.Fa, 0, a); else if (D(a, "readystatechange"), 
        4 == O(a)) {
            a.h = !1;
            try {
                const n = a.ba();
    ...
    k.bb = function() {
        this.B && (this.B = null, this.h.info("BP detection timeout reached."), this.h.info("Buffering proxy detected and switch to long-polling!"), 
        this.N = !1, this.L = !0, J(10), Ac(this), Rd(this));
    };
    function wc(a) {
    ...
    function uc(a, b) {
        ...
        if (0 != a.G) if (b.i) if (1 == d) {
            ...
            D(d, new Vb(d, c, b, e));
            Hc(a);
        } else Gc(a); else if (e = b.o, 3 == e || 0 == e && 0 < a.I || !(1 == d && Nd(a, b) || 2 == d && Bc(a))) switch (c && 0 < c.length && (b = a.i, 
        b.i = b.i.concat(c)), e) {
          case 1:
            Q(a, 5);
            break;
    ...
    function Q(a, b) {
        ...
        if (2 == b) {
            ...
            a.j && (c = null);
            var d = q(a.jb, a);
            c || (c = new U("//www.google.com/images/cleardot.gif"), l.location && "http" == l.location.protocol || Oc(c, "https"), 
            jc(c));
            nd(c.toString(), d);
        } else J(2);
        a.G = 0;
    ...
    k.jb = function(a) {
        a ? (this.h.info("Successfully pinged google.com"), J(2)) : (this.h.info("Failed to ping google.com"), 
        J(1));
    };
    function Ld(a) {
        a.G = 0;
    ...
    function Y(a, b) {
        ...
        (b = b && b.httpSessionIdParam) && !sa(b) && (this.g.D = b, a = this.h, null !== a && b in a && (a = this.h, 
        b in a && delete a[b]));
        this.j = new Z(this);
    }
    t(Y, C);
    ...
    esm.createWebChannelTransport = function() {
        return new Td;
    };
    esm.getStatEventTarget = function() {
        return Sb();
    };
    esm.ErrorCode = Wb;
    esm.EventType = Xb;
    ...
    esm.FetchXmlHttpFactory = pd;
    esm.WebChannel = $b;
    esm.XhrIo = X;
    const index_esm2017_D = "@firebase/firestore";
    /**
 * @license
 * Copyright 2017 Google LLC
 *
 ...
 */    class index_esm2017_C {
    ...
    index_esm2017_C.UNAUTHENTICATED = new index_esm2017_C(null), index_esm2017_C.GOOGLE_CREDENTIALS = new index_esm2017_C("google-credentials-uid"), 
    index_esm2017_C.FIRST_PARTY = new index_esm2017_C("first-party-uid"), index_esm2017_C.MOCK_USER = new index_esm2017_C("mock-user");
    /**
 * @license
 * Copyright 2017 Google LLC
 *
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */    const index_esm2017_N = new Logger("@firebase/firestore");
    function index_esm2017_O(t, ...e) {
        if (index_esm2017_N.logLevel <= index_esm2017_LogLevel.DEBUG) {
            const n = e.map(index_esm2017_B);
            index_esm2017_N.debug(`Firestore (${index_esm2017_x}): ${t}`, ...n);
        }
    }
    function index_esm2017_F(t, ...e) {
        if (index_esm2017_N.logLevel <= index_esm2017_LogLevel.ERROR) {
            const n = e.map(index_esm2017_B);
            index_esm2017_N.error(`Firestore (${index_esm2017_x}): ${t}`, ...n);
        }
    }
    function index_esm2017_B(t) {
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */    function index_esm2017_L(t = "Unexpected state") {
        const e = `FIRESTORE (${index_esm2017_x}) INTERNAL ASSERTION FAILED: ` + t;
        throw index_esm2017_F(e), new Error(e);
    }
    function index_esm2017_U(t, e) {
        t || index_esm2017_L();
    }
    /**
 * @license
 ...
 */
    ...
    class index_esm2017_Q extends FirebaseError {
        constructor(t, e) {
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */    class index_esm2017_W {
        constructor(t, e) {
            this.user = e, this.type = "OAuth", this.headers = new Map, this.headers.set("Authorization", `Bearer ${t}`);
        }
    }
    class index_esm2017_z {
        getToken() {
            return Promise.resolve(null);
        }
        invalidateToken() {}
        start(t, e) {
            t.enqueueRetryable((() => e(index_esm2017_C.UNAUTHENTICATED)));
        }
        shutdown() {}
    }
    class index_esm2017_J {
        constructor(t) {
            this.t = t, this.currentUser = index_esm2017_C.UNAUTHENTICATED, this.i = 0, this.forceRefresh = !1, 
            this.auth = null;
        }
        start(t, e) {
            let n = this.i;
            ...
            const r = () => {
                ...
                index_esm2017_O("FirebaseAuthCredentialsProvider", "Auth detected"), this.auth = t, 
                this.auth.addAuthTokenListener(this.o), r();
            };
            this.t.onInit((t => o(t))), setTimeout((() => {
                if (!this.auth) {
                    ...
                    t ? o(t) : (index_esm2017_O("FirebaseAuthCredentialsProvider", "Auth not yet detected"), 
                    i.resolve(), i = new j);
                }
            }), 0), r();
        }
        getToken() {
            ...
            return this.forceRefresh = !1, this.auth ? this.auth.getToken(e).then((e => this.i !== t ? (index_esm2017_O("FirebaseAuthCredentialsProvider", "getToken aborted due to token change."), 
            this.getToken()) : e ? (index_esm2017_U("string" == typeof e.accessToken), new index_esm2017_W(e.accessToken, this.currentUser)) : null)) : Promise.resolve(null);
        }
        invalidateToken() {
            this.forceRefresh = !0;
        }
        ...
        u() {
            ...
            return index_esm2017_U(null === t || "string" == typeof t), new index_esm2017_C(t);
        }
    }
    class index_esm2017_Y {
        constructor(t, e, n) {
            this.type = "FirstParty", this.user = index_esm2017_C.FIRST_PARTY, this.headers = new Map, 
            this.headers.set("X-Goog-AuthUser", e);
            const s = t.auth.getAuthHeaderValueForFirstParty([]);
            s && this.headers.set("Authorization", s), n && this.headers.set("X-Goog-Iam-Authorization-Token", n);
        }
    }
    class index_esm2017_X {
        constructor(t, e, n) {
            this.h = t, this.l = e, this.m = n;
        }
        getToken() {
            return Promise.resolve(new index_esm2017_Y(this.h, this.l, this.m));
        }
        start(t, e) {
            t.enqueueRetryable((() => e(index_esm2017_C.FIRST_PARTY)));
        }
        shutdown() {}
        invalidateToken() {}
    }
    ...
    class tt {
        ...
        start(t, e) {
            const n = t => {
                null != t.error && index_esm2017_O("FirebaseAppCheckTokenProvider", `Error getting App Check token; using placeholder token instead. Error: ${t.error.message}`);
                const n = t.token !== this.p;
                return this.p = t.token, index_esm2017_O("FirebaseAppCheckTokenProvider", `Received ${n ? "new" : "existing"} token.`), 
            ...
            const s = t => {
                index_esm2017_O("FirebaseAppCheckTokenProvider", "AppCheck detected"), this.appCheck = t, 
                this.appCheck.addTokenListener(this.o);
            };
            this.g.onInit((t => s(t))), setTimeout((() => {
                if (!this.appCheck) {
                    const t = this.g.getImmediate({
                        optional: !0
                    });
                    t ? s(t) : index_esm2017_O("FirebaseAppCheckTokenProvider", "AppCheck not yet detected");
                }
            }), 0);
        }
        getToken() {
            const t = this.forceRefresh;
            return this.forceRefresh = !1, this.appCheck ? this.appCheck.getToken(t).then((t => t ? (index_esm2017_U("string" == typeof t.token), 
            this.p = t.token, new index_esm2017_Z(t.token)) : null)) : Promise.resolve(null);
        }
        invalidateToken() {
            this.forceRefresh = !0;
        }
 ...
 */
    function nt(t) {
        ...
        if (e && "function" == typeof e.getRandomValues) e.getRandomValues(n); else for (let e = 0; e < t; e++) n[e] = Math.floor(256 * Math.random());
        return n;
    }
    /**
 * @license
 ...
 */
    class at {
        constructor(t, e, n) {
            void 0 === e ? e = 0 : e > t.length && index_esm2017_L(), void 0 === n ? n = t.length - e : n > t.length - e && index_esm2017_L(), 
    ...
    class ht extends at {
        ...
        static fromString(...t) {
            const e = [];
            for (const n of t) {
                if (n.indexOf("//") >= 0) throw new index_esm2017_Q(G.INVALID_ARGUMENT, `Invalid segment (${n}). Paths must not contain // in them.`);
 ...
 */
    ...
    class Qt {
        constructor(t, e, n, s) {
            ...
            let i = 1;
            for (;!t.isEmpty(); ) if (i = e ? n(t.key, e) : 1, e && s && (i *= -1), i < 0) t = this.isReverse ? t.left : t.right; else {
                if (0 === i) {
    ...
    class jt {
        ...
        check() {
            if (this.isRed() && this.left.isRed()) throw index_esm2017_L();
            if (this.right.isRed()) throw index_esm2017_L();
            const t = this.left.check();
            if (t !== this.right.check()) throw index_esm2017_L();
 ...
 */ (this.binaryString);
    ...
    null && new RegExp(/^\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d(?:\.(\d+))?Z$/);
    function ee(t) {
 ...
 */
    var ns, ss;
    (ss = ns || (ns = {}))[ss.OK = 0] = "OK", ss[ss.CANCELLED = 1] = "CANCELLED", ss[ss.UNKNOWN = 2] = "UNKNOWN", 
    ss[ss.INVALID_ARGUMENT = 3] = "INVALID_ARGUMENT", ss[ss.DEADLINE_EXCEEDED = 4] = "DEADLINE_EXCEEDED", 
    ss[ss.NOT_FOUND = 5] = "NOT_FOUND", ss[ss.ALREADY_EXISTS = 6] = "ALREADY_EXISTS", 
    ss[ss.PERMISSION_DENIED = 7] = "PERMISSION_DENIED", ss[ss.UNAUTHENTICATED = 16] = "UNAUTHENTICATED", 
    ss[ss.RESOURCE_EXHAUSTED = 8] = "RESOURCE_EXHAUSTED", ss[ss.FAILED_PRECONDITION = 9] = "FAILED_PRECONDITION", 
    ss[ss.ABORTED = 10] = "ABORTED", ss[ss.OUT_OF_RANGE = 11] = "OUT_OF_RANGE", ss[ss.UNIMPLEMENTED = 12] = "UNIMPLEMENTED", 
 ...
 */
    ...
    const xi = [ "mutationQueues", "mutations", "documentMutations", "remoteDocuments", "targets", "owner", "targetGlobal", "targetDocuments", "clientMetadata", "remoteDocumentGlobal", "collectionParents", "bundles", "namedQueries" ], ki = [ "mutationQueues", "mutations", "documentMutations", "remoteDocumentsV14", "targets", "owner", "targetGlobal", "targetDocuments", "clientMetadata", "remoteDocumentGlobal", "collectionParents", "bundles", "namedQueries", "documentOverlays" ], Mi = ki;
    /**
 * @license
 * Copyright 2020 Google LLC
 *
 ...
 */
    class ur {
        ...
        oe(t, e) {
            ...
            this.fe(e)) : "arrayValue" in t ? (this.me(t.arrayValue, e), this.fe(e)) : index_esm2017_L();
        }
        le(t, e) {
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */ Vr.DEFAULT_COLLECTION_PERCENTILE = 10, Vr.DEFAULT_MAX_SEQUENCE_NUMBERS_TO_COLLECT = 1e3, 
    Vr.DEFAULT = new Vr(41943040, Vr.DEFAULT_COLLECTION_PERCENTILE, Vr.DEFAULT_MAX_SEQUENCE_NUMBERS_TO_COLLECT), 
 ...
 */    class ru {
        constructor(t, e, n = 1e3, s = 1.5, i = 6e4) {
            ...
            this.mo = null, this.yo = Date.now(), this.reset();
        }
        reset() {
            this.wo = 0;
        }
        ...
        Io(t) {
            ...
            s > 0 && index_esm2017_O("ExponentialBackoff", `Backing off for ${s} ms (base delay: ${this.wo} ms, delay with jitter: ${e} ms, last attempt: ${n} ms ago)`), 
            this.mo = this.js.enqueueAfterDelay(this.timerId, s, (() => (this.yo = Date.now(), 
            t()))), this.wo *= this.fo, this.wo < this.lo && (this.wo = this.lo), this.wo > this._o && (this.wo = this._o);
        }
        Eo() {
 ...
 */
    class Lu {
        ...
        cancel(t) {
            null !== this.timerHandle && (this.clearTimeout(), this.deferred.reject(new index_esm2017_Q(G.CANCELLED, "Operation cancelled" + (t ? ": " + t : ""))));
        }
        handleDelayElapsed() {
    ...
    function Uu(t, e) {
        if (index_esm2017_F("AsyncQueue", `${e}: ${t}`), Nt(t)) return new index_esm2017_Q(G.UNAVAILABLE, `${e}: ${t}`);
        throw t;
    }
    /**
 * @license
 ...
 * limitations under the License.
 */
    class index_esm2017_Jc {
        constructor(t, e, n, s) {
            this.authCredentials = t, this.appCheckCredentials = e, this.asyncQueue = n, this.databaseInfo = s, 
            this.user = index_esm2017_C.UNAUTHENTICATED, this.clientId = st.I(), this.authCredentialListener = () => Promise.resolve(), 
            this.appCheckCredentialListener = () => Promise.resolve(), this.authCredentials.start(n, (async t => {
                index_esm2017_O("FirestoreClient", "Received user=", t.uid), await this.authCredentialListener(t), 
                this.user = t;
            })), this.appCheckCredentials.start(n, (t => (index_esm2017_O("FirestoreClient", "Received new app check token=", t), 
        ...
        verifyNotTerminated() {
            if (this.asyncQueue.isShuttingDown) throw new index_esm2017_Q(G.FAILED_PRECONDITION, "The client has already been terminated.");
        }
        terminate() {
    ...
    const index_esm2017_ma = new Map;
    /**
 * @license
 * Copyright 2017 Google LLC
 *
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */    function index_esm2017_ya(t, e, n, s) {
        if (!0 === e && !0 === s) throw new index_esm2017_Q(G.INVALID_ARGUMENT, `${t} and ${n} cannot be used together.`);
    }
    /**
 * @license
 * Copyright 2020 Google LLC
 ...
 * limitations under the License.
 */
    class index_esm2017_Ra {
        constructor(t) {
            var e;
            if (void 0 === t.host) {
                if (void 0 !== t.ssl) throw new index_esm2017_Q(G.INVALID_ARGUMENT, "Can't provide ssl option if host option is not set");
                this.host = "firestore.googleapis.com", this.ssl = true;
            } else this.host = t.host, this.ssl = null === (e = t.ssl) || void 0 === e || e;
            if (this.credentials = t.credentials, this.ignoreUndefinedProperties = !!t.ignoreUndefinedProperties, 
            void 0 === t.cacheSizeBytes) this.cacheSizeBytes = 41943040; else {
                if (-1 !== t.cacheSizeBytes && t.cacheSizeBytes < 1048576) throw new index_esm2017_Q(G.INVALID_ARGUMENT, "cacheSizeBytes must be at least 1048576");
                this.cacheSizeBytes = t.cacheSizeBytes;
            }
            this.experimentalForceLongPolling = !!t.experimentalForceLongPolling, this.experimentalAutoDetectLongPolling = !!t.experimentalAutoDetectLongPolling, 
            this.useFetchStreams = !!t.useFetchStreams, index_esm2017_ya("experimentalForceLongPolling", t.experimentalForceLongPolling, "experimentalAutoDetectLongPolling", t.experimentalAutoDetectLongPolling);
        }
        isEqual(t) {
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */    class index_esm2017_ba {
        constructor(t, e, n) {
            this._authCredentials = e, this._appCheckCredentials = n, this.type = "firestore-lite", 
            this._persistenceKey = "(lite)", this._settings = new index_esm2017_Ra({}), this._settingsFrozen = !1, 
            t instanceof ue ? this._databaseId = t : (this._app = t, this._databaseId = function(t) {
                if (!Object.prototype.hasOwnProperty.apply(t.options, [ "projectId" ])) throw new index_esm2017_Q(G.INVALID_ARGUMENT, '"projectId" not provided in firebase.initializeApp.');
                return new ue(t.options.projectId);
            }(t));
        }
        get app() {
            if (!this._app) throw new index_esm2017_Q(G.FAILED_PRECONDITION, "Firestore was not initialized using the Firebase SDK. 'app' is not available");
            return this._app;
        }
        get _initialized() {
        ...
        _setSettings(t) {
            if (this._settingsFrozen) throw new index_esm2017_Q(G.FAILED_PRECONDITION, "Firestore has already been started and its settings can no longer be changed. You can only modify settings before calling any other methods on a Firestore object.");
            this._settings = new index_esm2017_Ra(t), void 0 !== t.credentials && (this._authCredentials = function(t) {
                if (!t) return new index_esm2017_z;
                switch (t.type) {
                  case "gapi":
                    const e = t.client;
                    return index_esm2017_U(!("object" != typeof e || null === e || !e.auth || !e.auth.getAuthHeaderValueForFirstParty)), 
                    new index_esm2017_X(e, t.sessionIndex || "0", t.iamToken || null);

                  case "provider":
                    return t.client;

                  default:
                    throw new index_esm2017_Q(G.INVALID_ARGUMENT, "makeAuthCredentialsProvider failed due to invalid credential type");
                }
            }(t.credentials));
        }
        _getSettings() {
        ...
        _terminate() {
            return function(t) {
                const e = index_esm2017_ma.get(t);
                e && (index_esm2017_O("ComponentProvider", "Removing Datastore"), index_esm2017_ma.delete(t), 
                e.terminate());
 ...
 * limitations under the License.
 */
    class index_esm2017_Ma {
        constructor() {
            this.Mc = Promise.resolve(), this.Oc = [], this.Fc = !1, this.$c = [], this.Bc = null, 
            this.Lc = !1, this.Uc = !1, this.qc = [], this.So = new ru(this, "async_queue_retry"), 
            this.Kc = () => {
                const t = su();
                t && index_esm2017_O("AsyncQueue", "Visibility state changed to " + t.visibilityState), 
                this.So.Eo();
            };
            const t = su();
        ...
        async jc() {
            if (0 !== this.Oc.length) {
                try {
                    ...
                    if (!Nt(t)) throw t;
                    index_esm2017_O("AsyncQueue", "Operation failed with retryable error: " + t);
                }
 ...
 * limitations under the License.
 */ (t);
                throw index_esm2017_F("INTERNAL UNHANDLED ERROR: ", e), t;
            })).then((t => (this.Lc = !1, t))))));
            return this.Mc = e, e;
        }
        ...
        Jc(t) {
            return this.zc().then((() => {
                ...
                for (const e of this.$c) if (e.skipDelay(), "all" !== t && e.timerId === t) break;
                return this.zc();
            }));
        }
        Yc(t) {
    ...
    class Ba extends index_esm2017_ba {
        constructor(t, e, n) {
            super(t, e, n), this.type = "firestore", this._queue = new index_esm2017_Ma, this._persistenceKey = "name" in t ? t.name : "[DEFAULT]";
        }
        _terminate() {
            return this._firestoreClient || index_esm2017_Ka(this), this._firestoreClient.terminate();
        }
    }
    function index_esm2017_Ka(t) {
        ...
        t._firestoreClient = new index_esm2017_Jc(t._authCredentials, t._appCheckCredentials, t._queue, s);
    }
    null && new RegExp("[~\\*/\\[\\]]");
    !function(t, e = !0) {
        !function(t) {
            ...
            const s = t.getProvider("app").getImmediate(), i = new Ba(s, new index_esm2017_J(t.getProvider("auth-internal")), new tt(t.getProvider("app-check-internal")));
            return n = Object.assign({
                useFetchStreams: e
            }, n), i._setSettings(n), i;
        }), "PUBLIC")), registerVersion(index_esm2017_D, "3.4.14", t), registerVersion(index_esm2017_D, "3.4.14", "esm2017");
    }();
    ...
    if (document.querySelector(".page-homepage")) {
        ...
        signUp.addEventListener("click", (e => {
            ...
            if (email.match(validRegex)) createUserWithEmailAndPassword(auth, email, password).then((userCredential => {
                ...
                if ("auth/email-already-in-use" == errorCode) errorMessage = "Email already in use"; else if ("auth/weak-password" == errorCode) errorMessage = "Password should be at least 6 characters"; else if ("auth/internal-error" == errorCode) errorMessage = errorValidate; else if ("auth/invalid-email" == errorCode) errorMessage = "Invalid email"; else if ("auth/missing-email" == errorCode) errorMessage = "Missing email"; else error.code;
                modalVisible(errorMessage, isError());
            })); else modalVisible(errorValidate, isError());
        }));
        login.addEventListener("click", (e => {
            ...
            signInWithEmailAndPassword(auth, email, password).then((userCredential => {
                ...
                if ("auth/user-not-found" == errorCode) errorMessage = "User not found"; else if ("auth/weak-password" == errorCode) errorMessage = "Password should be at least 6 characters"; else if ("auth/internal-error" == errorCode) errorMessage = errorValidate; else if ("auth/invalid-email" == errorCode) errorMessage = "Invalid email"; else if ("auth/missing-email" == errorCode) errorMessage = "Missing email"; else if ("auth/wrong-password" == errorCode) errorMessage = "Wrong password"; else if ("auth/too-many-requests" == errorCode) errorMessage = "Too many requests"; else error.code;
                modalVisible(errorMessage, isError());
            }));
        }));
